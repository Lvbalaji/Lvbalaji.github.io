...
### ⚠️ Preconditions
1. A valid user account (`wiener`) to generate a baseline token.
2. The application does not enforce signature checks on the backend.
...
### ⚠️ Preconditions
1. The application does not filter or reject tokens with `alg: none`.
...
2.  **Modify Header:**
    In the JWT Header (Inspector or JSON tab), change the `alg` parameter to `none`.
...
### ⚠️ Preconditions
1. The token is signed using HMAC (HS256).
2. The secret exists in common wordlists (like `rockyou.txt`).
...
### ⚠️ Preconditions
1. The server processes the `jwk` header parameter.
2. No whitelist is enforced.

### 🚨 Exploitation Steps

1.  **Generate Malicious Key:**
    In Burp, go to the **JWT Editor Keys** tab. Click **New RSA Key** -> **Generate**. Make sure the `kid` matches the original if necessary.
...
1.  **Host the Key:**
    Generate a new RSA Key in Burp. Copy the **Public Key as JWK**.
...
### ⚠️ Preconditions
1. The server runs on a Linux-like system where `/dev/null` exists.
...
3.  **Sign & Execute:**
    Change `sub` to `administrator`.
    Sign the token using your "Null" key (`AA==`).
...
### ⚠️ Preconditions
1. The server exposes its public key (e.g., via `/jwks.json`).
2. The implementation allows switching `alg` from RS256 to HS256.
